...
export default function CreateRoomDialog({ onClose }: CreateRoomDialogProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-start justify-center px-6 pt-[11vh]">
      <div className="w-full max-w-105 rounded-2xl border border-white/9 bg-[#141414] p-6 shadow-[0_32px_64px_rgba(0,0,0,0.7)]">
        {/* Header */}
        <div className="mb-6 flex items-start justify-between">
          ...
          <button
            onClick={handleClose}
            className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-white/30 transition-all duration-150 hover:bg-white/6 hover:text-white/60"
          >
        ...
        <div className="mb-5">
          ...
          <input
            ...
            maxLength={60}
            autoFocus
            className="w-full rounded-xl border border-white/9 bg-white/4 px-4 py-3 font-sans text-[14px] font-normal text-[#F5F0E8] placeholder-white/20 transition-all duration-200 outline-none focus:border-[#F5A623]/40 focus:bg-white/6 focus:shadow-[0_0_0_3px_rgba(245,166,35,0.08)]"
          />
        </div>
              ...
              <button
                ...
                className={[
                  ...
                  threshold === val
                    ...
                    : 'border border-white/8 bg-white/4 text-white/40 hover:border-white/20 hover:text-white/70',
                ].join(' ')}
              >
                {val}
              </button>
        ...
        {/* Error */}
        {error && (
          <div className="mb-4 rounded-xl border border-red-500/20 bg-red-500/8 px-3.5 py-2.5">
        ...
        <div className="flex gap-3">
          <button
            onClick={handleClose}
            disabled={loading}
            className="flex-1 rounded-xl border border-white/8 bg-transparent py-3 font-sans text-[14px] font-normal text-white/40 transition-all duration-150 hover:border-white/20 hover:text-white/60 disabled:opacity-40"
          >
            Cancel
          </button>
          <button
